...
import { CampData } from '../types/types';

function Camps() {

    const [data, setData] = useState<CampData | null>(null)
    const [filteredCamps, setFilteredCamps] = useState<CampData | null>(data);
    ...
    useEffect(() => {
        if (data) {
            const newFilteredCamp: any = data.filter((camp) => {
